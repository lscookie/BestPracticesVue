...
(function (factory) {
  ...
  var OrgChart = function (elem, opts, contentDom, fun, obj) {
  ...
  OrgChart.prototype = {
    ...
    tableClickHandler: function (event) {
      if(typeof this.fun === 'function'){
        this.fun.bind(this.obj)($(event.currentTarget.parentNode).attr('tdata'))
      }
    },
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        // 添加内容部分
        // 给dom绑定相对应的值
        let donstr = this.contentDom.outerHTML
        let index = 0
        let startInd = 0
        let endInd = 0
        let pname = ''
        let insertInd = 0
        while(index > -1){
          index = donstr.indexOf('pname', index + 1)
          if (index > -1){
            startInd = donstr.indexOf('\"', index + 1)
            if (startInd > -1){
              endInd = donstr.indexOf('\"', startInd + 1)
              if (endInd > startInd) {
                pname = donstr.substring(startInd + 1, endInd).trim()
                if (data.hasOwnProperty(pname)){
                  insertInd = donstr.indexOf('>', endInd + 1)
                  donstr = donstr.substr(0, insertInd + 1) + data[pname] + donstr.substr(insertInd + 1)
                }
              }
            }
          }
        }
        let domData = JSON.stringify(data)
        domData = JSON.parse(domData)
        delete domData.children
        $nodeDiv.attr('tdata', JSON.stringify(domData))
      ...
      $nodeDiv.on('click', 'table', this.tableClickHandler.bind(this));
  ...
  $.fn.orgchart = function (opts, contentDom, fun, obj) {
    return new OrgChart(this, opts, contentDom, fun, obj).init();
  };
